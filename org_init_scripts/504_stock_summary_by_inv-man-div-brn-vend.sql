...
begin
    ...
    return query
        with s1 as (select min(it.inventory_id)                            as s1id,
                    ...
                    from inv_txn as it
                             left join public.batch b on b.id = it.batch_id
                    where it.date <= as_on_date
                      and (case when array_length(br_ids, 1) > 0 then it.branch_id = any (br_ids) else true end)
                      and (case when array_length(div_ids, 1) > 0 then it.division_id = any (div_ids) else true end)
                      and (case when array_length(inv_ids, 1) > 0 then it.inventory_id = any (inv_ids) else true end)
                      and (case when array_length(man_ids, 1) > 0 then it.manufacturer_id = any (man_ids) else true end)
                      and (case when array_length(vend_ids, 1) > 0 then it.vendor_id = any (vend_ids) else true end)
...
begin
    ...
    return query
        with s1 as (select min(it.manufacturer_id)                         as s1id,
...
begin
    ...
    return query
        with s1 as (select min(it.division_id)                             as s1id,
...
begin
    ...
    return query
        with s1 as (select min(it.branch_id)                               as s1id,
...
begin
    ...
    return query
        with s1 as (select min(it.vendor_id)                               as s1id,
