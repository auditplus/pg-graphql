    ...
    foreach _item in array _items
        loop
            ...
            values (_item.id, _book_begin, _branch.id, _division.id, _division.name, _branch.name, _batch.id,
                    ...
                    _item.qty * _item.unit_conv * loose, _item.asset_amount, _batch.warehouse_id, _batch.warehouse_name,
                    true, _item.nlc)
            on conflict (id) do update
                set inventory_name    = excluded.inventory_name,
                    inventory_hsn     = excluded.inventory_hsn,
...
--         values (gen_random_uuid(), _book_begin, 16, 0.0, coalesce(asset_amt, 0), 'Inventory Asset', array ['STOCK'],
--                 _branch.id, _branch.name, true);
--     end if;
