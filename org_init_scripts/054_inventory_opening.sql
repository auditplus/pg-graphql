...
begin
    if items is null or array_length(items, 1) = 0 then
        delete from inv_txn where inventory_id = (input->>'inventory_id')::int and branch_id = (input->>'branch_id')::int and warehouse_id = (input->>'warehouse_id')::int and is_opening = true;
        delete from inventory_opening where inventory_id = (input->>'inventory_id')::int and branch_id = (input->>'branch_id')::int and warehouse_id = (input->>'warehouse_id')::int;
        delete from batch where inventory_id = (input->>'inventory_id')::int and branch_id = (input->>'branch_id')::int and warehouse_id = (input->>'warehouse_id')::int and entry_type = 'OPENING';
        select sum(asset_amount)
        into asset_amt
        from inv_txn
        where inv_txn.branch_id = (input->>'branch_id')::int
          and is_opening = true;
        update ac_txn set debit = coalesce(asset_amt, 0) where branch_id = (input->>'branch_id')::int and account_id = 16 and is_opening = true;
        return true;
    end if;
    ...
    foreach item in array items
        loop
            ...
            insert into inventory_opening (id, inventory_id, branch_id, warehouse_id, unit_id, unit_conv, qty, nlc,
                                           cost, rate,
                                           is_loose_qty, landing_cost, mrp, s_rate, batch_no, expiry,
                                           asset_amount)
            values (coalesce(item.id,gen_random_uuid()), (input->>'inventory_id')::int, (input->>'branch_id')::int, (input->>'warehouse_id')::int, item.unit_id, item.unit_conv, item.qty, item.nlc, item.cost, item.rate,
            ...
            values (item.id, (input->>'inventory_id')::int, coalesce(inv.reorder_inventory_id, (input->>'inventory_id')::int), inv.name, inv.hsn_code, (input->>'branch_id')::int, br.name, (input->>'warehouse_id')::int,
            ...
            values (item.id, en_date, (input->>'branch_id')::int, div.id, div.name, br.name, bat.id, (input->>'inventory_id')::int, coalesce(inv.reorder_inventory_id, (input->>'inventory_id')::int),
                    inv.name, inv.hsn_code, inv.manufacturer_id, inv.manufacturer_name,
                    item.qty * item.unit_conv * loose, item.asset_amount, bat.warehouse_id, bat.warehouse_name,
                    true, item.nlc)
            on conflict (id) do update
                set inventory_name    = excluded.inventory_name,
                    inventory_hsn     = excluded.inventory_hsn,
    ...
    if not FOUND then
        insert into ac_txn(id, date, account_id, credit, debit, account_name, base_account_types, branch_id, branch_name,
                           is_opening)
        values (gen_random_uuid(), en_date, 16, 0.0, coalesce(asset_amt, 0), 'Inventory Asset',array ['STOCK'], (input->>'branch_id')::int, br.name,
                true);
    end if;
