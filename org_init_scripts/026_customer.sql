...
(
    ...
    name                     text             not null
        constraint customer_name_min_length check (char_length(trim(name)) > 0),
    short_name               text,
    pan_no                   text
        constraint customer_pan_no_invalid check (pan_no ~ '^[a-zA-Z]{5}[0-9]{4}[a-zA-Z]$'),
    aadhar_no                text
        constraint customer_aadhar_no_invalid check (aadhar_no ~ '^[2-9][0-9]{3}[0-9]{4}[0-9]{4}$'),
    gst_reg_type             typ_gst_reg_type not null,
    gst_location_id          text,
    gst_no                   text
        constraint customer_gst_no_invalid check (check_gst_no(gst_no)),
    mobile                   text,
