...
(
    ...
    gift_voucher_account int                   not null,
...
declare
    ...
    v_expiry_type typ_gift_voucher_expiry := create_gift_voucher.expiry_type::typ_gift_voucher_expiry;
    j              json;
...
begin
    ...
    insert into gift_voucher(voucher, date, eff_date, branch, branch_name, issued, valid_from, valid_to, expiry,
                             expiry_type, amount, voucher_type, base_voucher_type, voucher_no, voucher_prefix,
                             voucher_fy, voucher_seq, ref_no, description, ac_trns, denominations, name,
                             party_account, gift_voucher_account)
    values (v_voucher.id, v_voucher.date, v_voucher.eff_date, v_voucher.branch, v_voucher.branch_name,
            ...
            v_expiry_type, create_gift_voucher.amount, v_voucher.voucher_type,
    ...
    for j in select jsonb_array_elements(create_gift_voucher.denominations)
        loop
            insert into gift_coupon(name, amount, gift_voucher, branch, valid_from, valid_to, gift_voucher_account)
            select v_gift_voucher.name,
                   (j ->> 'amount')::float,
                   v_gift_voucher.id,
                   v_gift_voucher.branch,
                   v_gift_voucher.valid_from,
                   val_to,
                   v_gift_voucher.gift_voucher_account
...
begin
    ...
    into v_voucher
    from
        update_voucher(v_id := v_gift_voucher.voucher, date := v_gift_voucher.date, ref_no := v_gift_voucher.ref_no,
...
$$ language plpgsql security definer;
--##
create function delete_gift_voucher(v_id int)
    returns void as
$$
declare
    voucher_id int;
begin
    delete from gift_voucher where id = $1 returning voucher into voucher_id;
...
begin
    for j in select jsonb_array_elements(gift_coupons)
        loop
            ...
            from gift_coupon
            where id = (j ->> 'id')::int
              and gift_voucher_account = (j ->> 'account')::int
              and active = true
            returning gift_voucher into gift_voucher_id;
            if not FOUND then
